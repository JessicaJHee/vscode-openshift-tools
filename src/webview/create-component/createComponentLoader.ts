    ...
    static async messageHandler(message: Message) {
        switch (message.action) {
            ...
            case 'getRecommendedDevfileFromGit': {
                ...
                const cloneProcess: CloneProcess = await clone(message.data.url, tmpFolder.fsPath, message.data.branch, message.data.contextDir);
...
function clone(url: string, location: string, branch?: string, contextDir?: string): Promise<CloneProcess> {
    ...
    command = contextDir ? command + ` ${contextDir}` : command;
